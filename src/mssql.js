...
module.exports = function (RED) {
    ...
    function connection(config) {
        ...
        async function executeWithRetry(execFunction, maxRetries = 3, delay = 1000) {
            let retries = 0;
        ...
        node.connect = function () {
            ...
            node.poolConnect = node.pool.connect().then(() => {
                node.status({
                    fill: 'green',
                    shape: 'dot',
                    text: 'connected'
        ...
        node.execSql = async function (queryMode, sqlQuery, params, paramValues, callback) {
            ...
            try {
                ...
                node.log(`Error executing query on server: ${node.config.server}, database: ${node.config.database}, port: ${node.config.options.port}, user: ${node.config.user}`);
                console.error(e);
                node.poolConnect = null;
                callback(e);
            }
    ...
    function mssql(config) {
        ...
        node.processError = function (err, msg) {
            ...
            if (typeof err === 'string') {
                ...
                // Make an error object from the err. NOTE: We can't just assign err to msg.error as a promise
